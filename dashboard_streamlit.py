...
def to_float(v):
    """Converte Decimal ou qualquer tipo numerico para float."""
    if v is None:
        return 0.0
    return float(v)
...
def load_monthly():
    return conn.execute("""
        SELECT d.year AS ano, d.month AS mes, d.month_name AS nome_mes,
...
def load_kpis():
    row = conn.execute("""
        ...
        WHERE order_status NOT IN ('canceled','unavailable')
    """).fetchone()
    # garante tudo float/int
    return tuple(to_float(v) for v in row)
...
# converte colunas numericas dos DataFrames
for df in [monthly, cats, states, payments, reviews]:
...
with st.sidebar:
    ...
    st.caption("DW sobre o dataset Olist (Kaggle). FATEC Jundiaí.")
...
fig_line.add_trace(go.Scatter(
    x=monthly_f['periodo'], y=monthly_f[metrica].astype(float),
...
# ── Categorias + Pagamento/Avaliações ────────────────────────────
...
with col_a:
    ...
    fig_bar.update_layout(
        ...
        xaxis=dict(gridcolor='#f0f0f0',
                   tickprefix="R$ " if metric_cat in ['receita','ticket_medio'] else "")
    )
...
with col_b:
    ...
    payments_plot = payments.copy()
    payments_plot['label']   = payments_plot['tipo'].map(pay_labels).fillna(payments_plot['tipo'])
    payments_plot['receita'] = payments_plot['receita'].astype(float)

    fig_pie = px.pie(payments_plot, values='receita', names='label',
    ...
    reviews_plot = reviews.copy()
    total_rev = int(reviews_plot['qtd'].sum())
    reviews_plot['pct'] = (reviews_plot['qtd'] / total_rev * 100).round(1)

    fig_rev = px.bar(reviews_plot, x='nota', y='pct',
        color='nota', color_discrete_sequence=rev_colors,
        labels={'nota': 'Nota', 'pct': '% avaliações'},
        text=reviews_plot['pct'].apply(lambda x: f"{x:.0f}%"))
...
with col_ord:
    ...
    sort_field = st.selectbox("Ordenar por:", options=list(sort_opts.keys()),
                              format_func=lambda x: sort_opts[x])
with col_top:
    n_states = st.slider("Quantos estados", 5, 27, 15)

states_plot = states.copy()
for col in ['receita','ticket_medio','prazo_dias','pct_prazo','avaliacao']:
    states_plot[col] = pd.to_numeric(states_plot[col], errors='coerce')

states_sorted = states_plot.sort_values(sort_field, ascending=False).head(n_states)
